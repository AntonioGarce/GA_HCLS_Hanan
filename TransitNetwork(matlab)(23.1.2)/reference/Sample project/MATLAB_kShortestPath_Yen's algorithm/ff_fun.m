function [FFVAL] = ff_fun(solution,demandMatrix,travelTimeMatrix,w1,w2,w3,omega1,omega2,omega3,P1,P2)
    ...
    canBeT0 = zeros(numnodes,numnodes);         %matrix which shows that we can trip between two nodes by direct trip
    canBeT1 = zeros(numnodes,numnodes);         %matrix which shows that we can trip between two nodes by one transfer
    canBeT2 = zeros(numnodes,numnodes);         %matrix which shows that we can trip between two nodes by two transfer
