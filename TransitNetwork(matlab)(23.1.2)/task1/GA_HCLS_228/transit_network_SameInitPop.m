...
len=0;
for k = 1:length(currSolution)
    len=len+length(currSolution{k});
end
disp(len);
currSol_ind = {};
...
%     end
    ...
    for pop_i = 1:population
        fitMatrix(pop_i) = 1/(1+3*(objMatrix(pop_i)-minFit)/(maxFit-minFit+0.001));
    end
