...
%             end
%             dstTime = 0;
%             for k=1:length(dstnode)-1
%                 dstTime = dstTime + originTravelTimeMatrix(dstnode(k),dstnode(k+1));
%             end
            % max 
...
%                 originTravelTimeMatrix(srcnode(end),dstnode(end))]) + srcTime + dstTime;
...
for genIter = 1:gen_num
    ...
    pause(0.2)
    subplot(2,1,2);
    title("best:"+num2str(bestFitValFull)+" mean:"+num2str(meanFitValFull));
    plot(genIter,bestFitValFull,'bo-',genIter,meanFitValFull,'rd--');
    axis([1 gen_num -30 10]);
    hold on
    pause(0.2)
    saveas(gcf,'aa.fig')
